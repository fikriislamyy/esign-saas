...
class TemplateUploadTest extends TestCase
{
    ...
    public function test_a_pdf_of_exactly_5mb_is_accepted(): void
    {
        $response = $this->actingAs($this->user)->post('/templates', [
            'file' => $this->pdf('template.pdf', 5 * 1024 * 1024),
        ]);

        $response->assertSessionHasNoErrors();
        $this->assertDatabaseHas('templates', ['name' => 'template']);
    }

    public function test_a_pdf_one_byte_over_5mb_is_rejected(): void
    {
        $response = $this->actingAs($this->user)->post('/templates', [
            'file' => $this->pdf('template.pdf', 5 * 1024 * 1024 + 1),
        ]);
    ...
    {
        $file = UploadedFile::fake()->createWithContent('template.pdf', str_repeat("\0", 1024));
    ...
    {
        $response = $this->actingAs($this->user)->post('/templates', [
            'file' => $this->pdf('template.txt', 1024),
        ]);
