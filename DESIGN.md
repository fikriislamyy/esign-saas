...
**Theme:** dark (primary) · light (counterpart — see [Light Theme](#light-theme))
...
The same precision instrument, moved into daylight. Light mode is not an inversion — it is a re-derivation under one constraint the dark system never had to answer: **on a white canvas, acid lime has nowhere to hide.**

Everything else transfers intact. Hairline borders still do the work shadows would do elsewhere. Type still sits in the 400–510 weight band at tight tracking. The radius vocabulary is unchanged (2 / 4 / 6 / 12 / 9999px). Density stays compact. What changes is the surface ramp, the grey ramp's two endpoints, and how the accent earns its contrast.

### The accent problem

`#e4f222` measures **1.23:1** against white. As a fill on a light canvas it has effectively no edge — the button dissolves into the page. But acid lime **with near-black text on top** measures **16.2:1**, identical to its dark-mode reading, because that contrast pair never involved the page at all.

So light mode splits the accent into two roles:

| Role | Dark | Light | Why |
|------|------|-------|-----|
| Filled CTA | `#e4f222` fill, `#08090a` text | `#e4f222` fill, `#08090a` text, **+ 1px `#c9d600` border** | Fill/text pair is already 16.2:1; the border is what gives the button an edge against white |
| Accent as text, icon, thin rule, active indicator | `#e4f222` | `#727a00` (Lime Shade) | 4.7:1 on white — same hue (64°), passes AA where the bright lime cannot |

**Never use `#e4f222` for text, a 1px underline, or a focus ring on a light surface.** It will read as a smudge. That is what Lime Shade exists for.

### Tokens — Colors (Light)

Ash and Fog carry over unchanged — their contrast on white lands in the same role band they occupy on void. Paper and Void swap jobs rather than disappearing.

| Name | Value | Token | Role |
|------|-------|-------|------|
| Paper | `#ffffff` | `--color-paper` | Card surfaces, elevated panels — reused from the dark palette, promoted from "max-contrast text" to "surface" |
| Daylight | `#f7f8f9` | `--color-daylight` | Page canvas, full-bleed backgrounds — the faint grey everything sits on |
| Powder | `#f1f2f4` | `--color-powder` | Interactive surface tint, ghost button fills, hover states |
| Chalk | `#e8e9ec` | `--color-chalk` | Hairline borders, dividers, card edges — the light-mode counterpart to Graphite |
| Quartz | `#d3d5da` | `--color-quartz` | Higher-contrast hairlines, section separators — counterpart to Smoke |
| Fog | `#8a8f98` | `--color-fog` | Placeholder copy, disabled text, inactive icons (3.3:1 — never body text) |
| Ash | `#62666d` | `--color-ash` | Muted body text, secondary metadata (5.8:1) |
| Graphite | `#23252a` | `--color-graphite` | Primary body text (15.3:1) — promoted from "border" in dark to "body" in light |
| Void | `#08090a` | `--color-void` | Primary headings, hero type, max-contrast emphasis |
| Acid Lime | `#e4f222` | `--color-acid-lime` | Filled CTA background **only** — requires the Lime Edge border on light surfaces |
| Lime Edge | `#c9d600` | `--color-lime-edge` | 1px border on the acid-lime CTA — defines the button's silhouette against white |
| Lime Shade | `#727a00` | `--color-lime-shade` | Accent as text, icon fill, active nav indicator, focus ring (4.7:1) |

#### Chromatic accents

The five supporting chromatics hold up as **fills** (badge backgrounds carrying dark text) but all fail AA as **text** on white — Iris Violet misses by a hair at 4.5:1, the rest by a wide margin. Each gets a darkened light-mode twin for typographic use:

| Name | Dark | Light (text) | Contrast on white |
|------|------|--------------|-------------------|
| Pulse Green | `#27a644` | `#1a7a30` | 5.4:1 |
| Coral Red | `#eb5757` | `#c23434` | 5.5:1 |
| Signal Teal | `#02b8cc` | `#027d8c` | 4.9:1 |
| Iris Violet | `#6366f1` | `#4f52d9` | 5.9:1 |
| Lavender | `#8b5cf6` | `#7340e6` | 5.8:1 |

Keep the original values when the colour is a background with dark text on it. Swap to the darkened twin the moment the colour becomes the ink.

### Surfaces (Light)

The dark ramp elevates by getting lighter (`#08090a → #0f1011 → #161718 → #23252a`). Light elevates by getting **whiter**, with the canvas sitting one step down in grey — so a card still reads as lifted off the page without a shadow.

| Level | Name | Value | Purpose |
|-------|------|-------|---------|
| 0 | Daylight | `#f7f8f9` | Page canvas — the default full-bleed background |
| 1 | Paper | `#ffffff` | Card surfaces, product screenshot frames, nav containers |
| 2 | Paper + Quartz | `#ffffff` + 1px `#d3d5da` | Elevated panels — same fill, stronger edge (light mode cannot go "whiter than white", so elevation past level 1 is carried entirely by border weight) |
| 3 | Powder | `#f1f2f4` | Interactive surface tint, ghost button fills, border-adjacent backgrounds |

### Elevation (Light)

Same philosophy, inverted values. Borders remain the primary separator; shadows stay near-invisible and exist only to lift a floating element off the canvas.

| Name | Value | Token |
|------|-------|-------|
| sm | `rgba(8, 9, 10, 0.06) 0px 2px 4px 0px` | `--shadow-sm` |
| subtle | `rgb(232, 233, 236) 0px 0px 0px 1px inset` | `--shadow-subtle` |
| subtle-2 | `rgba(8, 9, 10, 0.08) 0px 0px 0px 1px` | `--shadow-subtle-2` |
| xl | `rgba(8, 9, 10, 0.10) 0px 4px 32px 0px` | `--shadow-xl` |

Dark mode's `--shadow-md` (inset 12px black glow) has no light-mode equivalent — drop it rather than translating it. A dark inner glow on white reads as dirt.

### Component adjustments

Only the components whose values actually move are listed. Everything not here — radii, padding, type sizes, weights, tracking — is unchanged from the dark spec.

| Component | Change |
|-----------|--------|
| **Primary Action Button** | `#e4f222` fill and `#08090a` text both hold. Add `1px solid #c9d600`. |
| **Nav Text Button** | Text `#d0d6e0` → `#23252a`. |
| **Pill Button** | Fill `rgba(255,255,255,0.05)` → `rgba(8,9,10,0.05)`; text → `#23252a`. |
| **Ghost / Outline Button** | Border `#23252a` → `#e8e9ec`; text → `#23252a`. |
| **Sign-up Button** | Inverts fully: fill `#ffffff` → `#08090a`, text `#08090a` → `#ffffff`. It stays the second-highest-contrast element after the CTA. |
| **Card (Screenshot Frame)** | Fill `#0f1011` → `#ffffff`; inset border `rgb(35,37,42)` → `rgb(232,233,236)`. Still no outer shadow. |
| **Card (Subtle)** | Fill `rgba(255,255,255,0.02)` → `rgba(8,9,10,0.02)`; shadow → `rgba(8,9,10,0.06) 0 2px 4px`. |
| **Text Input** | Fill → `rgba(8,9,10,0.02)`; border → `rgba(8,9,10,0.10)`; text → `#23252a`. Focus: border darkens to `#23252a`. |
| **Badge / Status Tag** | Fill → `rgba(8,9,10,0.05)`; text `#8a8f98` → `#62666d`. Colour-coded variants use the darkened twins above. |
| **Logo Mark** | `#ffffff` → `#08090a`. |
| **Logo Bar** | Grey range `#8a8f98–#d0d6e0` → `#8a8f98–#62666d` (logos must darken, not lighten, to stay legible). |
| **Hero Gradient Floor** | `rgb(8,9,10) → rgb(208,214,224)` becomes `rgb(255,255,255) → rgb(215,219,226)` — a soft grey wash downward instead of a light wash upward. |

### Do's and Don'ts (Light)

#### Do
- Pair `#e4f222` with `#08090a` text and a `#c9d600` border — that trio is the only correct filled-CTA recipe on a light canvas.
- Use `#727a00` (Lime Shade) any time the accent is ink rather than fill.
- Keep body copy in `#23252a` and drop to `#62666d` for secondary — the same two-step ramp the dark theme uses, just re-anchored.
- Let border weight, not fill, carry elevation past level 1 — white cannot get whiter.
- Swap chromatic accents to their darkened twins the moment they become text.

#### Don't
- Do not use `#e4f222` as text, icon fill, underline, or focus ring on light surfaces — 1.23:1 against the page.
- Do not translate `--shadow-md` (the inset dark glow) into light mode — it reads as a smudge on white.
- Do not reach for pure `#000000` as the text colour — `#08090a` carries the same faint blue cast that makes the dark theme's whites feel deliberate.
- Do not add drop shadows to compensate for the lower surface contrast. If a card is not separating, thicken its border to Quartz (`#d3d5da`); do not introduce ambient shadow.
- Do not lighten the customer logo strip to match the dark theme's `#d0d6e0` — logos must darken to stay legible.

### Agent Prompt Guide (Light)

**Quick Color Reference:**
- text (primary heading): `#08090a`
- text (body): `#23252a`
- text (muted): `#62666d`
- text (placeholder): `#8a8f98`
- background (canvas): `#f7f8f9`
- background (card): `#ffffff`
- border (hairline): `#e8e9ec`
- accent (CTA fill): `#e4f222` + `#c9d600` border
- accent (as text/icon): `#727a00`

**Example component prompts:**

1. **Hero headline block (light):** Full-bleed `#f7f8f9` canvas. Headline at 64px Inter Variable weight 510, colour `#08090a`, letter-spacing -0.022em, line-height 1.0. Subtext at 16px Inter weight 400, colour `#62666d`. Secondary link text in `#23252a` with arrow glyph.

2. **Acid-lime primary action button (light):** Background `#e4f222`, text `#08090a`, border 1px solid `#c9d600`, border-radius 6px, padding 10px 16px, Inter 14px weight 510, letter-spacing -0.011em. Only one per view.

3. **Product screenshot card (light):** Background `#ffffff`, border-radius 12px, inset border 1px `#e8e9ec` via box-shadow, padding 24px. No outer drop shadow.

4. **Status badge row (light):** Horizontal flex, 8px gap. Each badge: background `rgba(8,9,10,0.05)`, text `#62666d`, border-radius 4px, padding 0px 6px, Inter 12px weight 400. Colour-coded variants: `#1a7a30` success, `#c23434` error, `#4f52d9` tags.
...
Dark is the default above. This block overrides only what moves; every token not listed here
(type scale, spacing, radii, weights, layout) is shared across both themes.

```css
[data-theme="light"] {
  /* Palette additions */
  --color-daylight: #f7f8f9;
  --color-powder: #f1f2f4;
  --color-chalk: #e8e9ec;
  --color-quartz: #d3d5da;
  --color-lime-edge: #c9d600;
  --color-lime-shade: #727a00;

  /* Chromatic accents — darkened twins for typographic use */
  --color-pulse-green-text: #1a7a30;
  --color-coral-red-text: #c23434;
  --color-signal-teal-text: #027d8c;
  --color-iris-violet-text: #4f52d9;
  --color-lavender-text: #7340e6;
  ...
  /* Shadows — note: --shadow-md is intentionally unset in light mode */
  --shadow-sm: rgba(8, 9, 10, 0.06) 0px 2px 4px 0px;
  --shadow-subtle: rgb(232, 233, 236) 0px 0px 0px 1px inset;
  --shadow-subtle-2: rgba(8, 9, 10, 0.08) 0px 0px 0px 1px;
  --shadow-xl: rgba(8, 9, 10, 0.10) 0px 4px 32px 0px;
}
```
...
@theme {
  ...
  /* Light-theme palette additions — these generate bg-/text-/border- utilities
     and are theme-agnostic; the theme switch happens in the semantic layer below. */
  --color-daylight: #f7f8f9;
  --color-powder: #f1f2f4;
  --color-chalk: #e8e9ec;
  --color-quartz: #d3d5da;
...
#### Wiring the theme switch

Declare the palette once in `@theme` (above), then map semantic roles to it per theme.
Components reference only the semantic layer, so a theme swap never touches component code:
...
@theme inline {
  ...
  --color-ink: var(--text-body);
  --color-ink-strong: var(--text-primary);
...
:root {
  ...
  --text-muted: #8a8f98;
  --border-hairline: #23252a;
  --accent-ink: #e4f222;      /* lime reads as ink on void */
}
...
[data-theme="light"] {
  ...
  --text-muted: #62666d;
  --border-hairline: #e8e9ec;
  --accent-ink: #727a00;      /* Lime Shade — bright lime fails on white */
}
```

`--accent-ink` is the token that makes the accent split safe: components write
`text-accent-ink` once and get `#e4f222` on dark, `#727a00` on light, with no
conditional class. The raw `#e4f222` stays reserved for the CTA **fill** in both themes.
